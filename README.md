# Setting up AWS EC2 Instance with Ubuntu 20.04

This guide will walk you through the process of creating an AWS EC2 instance with Ubuntu 20.04.

## Prerequisites

1. **AWS Account:** Ensure you have an AWS account. If not, you can sign up [here](https://aws.amazon.com/).

2. **AWS CLI:** Install AWS CLI on your local machine. You can find installation instructions [here](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html).

3. **SSH Key Pair:** Create an SSH key pair to securely connect to your EC2 instance. If you don't have one, you can generate it using the following command:
...
3. Click on "Launch Instance" and select "Ubuntu Server 20.04 LTS" as your Amazon Machine Image (AMI).

4. Choose an instance type based on your requirements.

5. Configure instance details, add storage, and configure any additional settings as needed.

6. In the "Security Group" section, add above police.
...
2. Open a terminal and use the following command to connect to your EC2 instance:

    ```bash
    ssh -i YourKeyName.pem ubuntu@your-instance-ip
    ```

    Replace `YourKeyName.pem` with the path to your private key and `your-instance-ip` with your instance's public IP address.
...
1. **Get Github Action IP:**
   - Utilizes the `haythem/public-ip` action to retrieve the public IP address of the GitHub Actions runner.

2. **Add Github Actions IP to Security group:**
   - Uses the AWS CLI to authorize the GitHub Actions runner's IP address in the specified security group to allow SSH access to the production server.

3. **Deploy to prod server:**
   - Utilizes the `appleboy/ssh-action` to SSH into the production server and perform the deployment steps.
   - The deployment steps include navigating to the application directory, stashing changes, pulling the latest code from the "develop" branch, installing dependencies, and building the application.

4. **Remove Github Actions IP from security group:**
   - Uses the AWS CLI to revoke the GitHub Actions runner's IP address from the security group to restrict further access.

5. **Conditional Removal:**
   - The last step is set to run always (`if: always()`) to ensure that the GitHub Actions runner's IP address is removed from the security group, even if the previous steps fail.

## Secrets

Ensure the following secrets are configured in your GitHub repository:

- `AWS_SG_NAME`: Name of the AWS security group.
- `AWS_ACCESS_KEY_ID`: AWS access key ID for authorization.
- `AWS_SECRET_ACCESS_KEY`: AWS secret access key for authorization.
- `AWS_DEFAULT_REGION`: AWS region where the EC2 instance is located.
- `HOST_IP`: IP address of the production server.
- `SSH_KEY`: SSH private key for authenticating with the production server.
